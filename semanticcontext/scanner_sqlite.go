...
// scanSQLite queries duckdb_columns() for all user tables in the attached
// SQLite catalog and returns OSI datasets. SQLite attachments do not expose
// information_schema, so we use DuckDB's built-in metadata function instead.
func scanSQLite(ctx context.Context, q MetadataQuerier, catalog string, _ queryengine.DatabaseConfig) ([]Dataset, error) {
	query := fmt.Sprintf(`
