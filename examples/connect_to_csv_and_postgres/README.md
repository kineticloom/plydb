...
This example demonstrates querying across a local CSV file and a PostgreSQL database using nexus.
...
```bash
docker exec -it nexus-postgres psql -U nexus -d store
```
...
```bash
./nexus mcp --config examples/connect_to_csv_and_postgres/config.json --transport stdio
```
